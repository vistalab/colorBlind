%% s_ctConeSamplesForDisplays
%  Script to test display pixel shape effects on clearType cone sampled image SVM
...
for dist = distRange
    ...
    for i = 1:10
        ...
        try
            ...
            % LibSVM routine
            svmStruct = svmtrain(Y(ind(1:1.8*nSamples)),sparse(I_train(ind(1:1.8*nSamples),:)),'-t 1 -d 1 -q');
