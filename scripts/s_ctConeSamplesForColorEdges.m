...
if ~exist('letter','var'), letter = 's'; end
...
%% SVM Classifier
count     = 1;
if ~exist('distRange','var'), distRange = 0.3:0.1:1; end
...
for dist = distRange
    ...
    scene    = ctCreateBinaryFontScene(vDisp, dist, letter, fontSize, fontFamily, dpi);
    vcAddAndSelectObject(scene{2}); sceneWindow
    Img = ctConeSamples(scene{1},nSamples, sensor, [], updateMosaic, eyeMove); % update cone mosaic;
    ...
    Img = ctConeSamples(scene{2},nSamples, sensor, [], updateMosaic, eyeMove); % Update cone mosaic    
    % Transform Img_c to matrix [row*col, nSamples]
    [row,col,~] = size(Img);
