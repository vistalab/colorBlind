%% s_ctConeSamplesForFilter
%  Script to test clearType font filter effects on clearType cone sampled
%  image SVM classification accuracy
...
if ~exist('letter','var'), letter = 'g'; end
...
%% SVM Classifier
count     = 1;
if ~exist('distRange','var'), distRange = 0.4:0.1:1; end
...
for dist = distRange
    % Generate clearType samples
    scene    = ctCreateFontScene(vDisp, dist, letter, fontSize, fontFamily, true, dpi);
    ...
    % Scale each line to have unit power. Shall I really need to do this?
    %I_c = I_c ./ repmat(sum(I_c,2),[1 row*col]);
    
    % Generate non-clearType samples
    filter = [0 0 0; 0 0 0;  0.6 -0.2 0.6; 0 0 0;0 0 0];
    scene    = ctCreateFontScene(vDisp, dist, letter, fontSize, fontFamily, true, dpi,filter);
    % vcAddAndSelectObject(scene); sceneWindow
